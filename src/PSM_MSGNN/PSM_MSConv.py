...
from .get_phase_signed_magnetic_Laplacian import get_phase_signed_magnetic_Laplacian

class PSM_MSConv(MessagePassing):
    ...
    def __init__(self, in_channels:int, out_channels:int, K:int,
                 normalization:str='sym', bias:bool=True, cached: bool=False, **kwargs):
        kwargs.setdefault('aggr', 'add')
        super(PSM_MSConv, self).__init__(**kwargs)
    ...
    def __norm__(
        ...
        edge_index, edge_weight_real, edge_weight_imag = get_phase_signed_magnetic_Laplacian(
            edge_index=edge_index, edge_weight=edge_weight, normalization=normalization, dtype=dtype, num_nodes=num_nodes, return_lambda_max = False,
        )
    ...
    def forward(
        ...
        if not self.cached or self.cached_result is None:
            ...
            if self.normalization != 'sym' and lambda_max is None:
                ...
                _, _, _, lambda_max =  get_phase_signed_magnetic_Laplacian(
                edge_index=edge_index, edge_weight=edge_weight, dtype=None, return_lambda_max=True
            )
            #print(lambda_max)

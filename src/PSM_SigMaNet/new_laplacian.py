...
def get_Phase_Magnetic_Laplacian(edge_index: torch.LongTensor, gcn: bool, net_flow:bool, edge_weight: Optional[torch.Tensor] = None,
    ...
    squared_A_sym = A_sym.power(2)
    squared_A_skew_sym = A_skew_sym.power(2)
    sum_of_squared_A = squared_A_sym + squared_A_skew_sym
    true_A = sum_of_squared_A.power(0.5)
    #operation = diag + A_double + (scipy.sparse.csr_matrix.sign(np.abs(A) - np.abs(A.T)))*1j
    H = A_sym + A_skew_sym * 1j
    deg = np.array(true_A.sum(axis=0))[0] # out degree
...
def __norm__(
        ...
        """
        edge_index, edge_weight = remove_self_loops(edge_index, edge_weight)
        edge_index, edge_weight_real, edge_weight_imag = get_Phase_Magnetic_Laplacian(
...
def process_magnetic_laplacian(edge_index: torch.LongTensor, gcn: bool, net_flow:bool, x_real: Optional[torch.Tensor] = None, edge_weight: Optional[torch.Tensor] = None,
    ...
    if normalization != 'sym' and lambda_max is None:        
        _, _, _, lambda_max =  get_Phase_Magnetic_Laplacian(

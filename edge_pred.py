...
def parse_args():
    ...
    parser.add_argument(
        "--dataset", type=str, default="bitcoin_otc", help="data set selection"
    )
    parser.add_argument(
        "--split_prob",
    ...
    parser.add_argument("--method", type=str, default="PSMGNN", help="method name")
...
def main(args):
    ...
    if args.dataset in ["bitcoin_otc", "bitcoin_alpha", "epinions", "telegram"]:
    ...
    for i in range(args.runs):
        ...
        for epoch in range(args.epochs):
            ...
            train_acc, _, _, _ = scores(pred_label, y_train)
            ...
            pred_label = out.max(dim=1)[1]
            val_acc, _, _, _ = scores(pred_label, y_val)
        ...
        pred_label = out.max(dim=1)[1]
        pred_label_csv=pd.DataFrame(pred_label)
        pred_label_csv.save()
        test_acc, f_score, macro_fscore, micro_fscore = scores(pred_label, y_test)
